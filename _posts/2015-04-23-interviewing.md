...
1. 遍历字符串，依据空格找出每个单词的起始位置和终止位置；
2. 反转每个单词；
3. 反转整个字符串。  

<p> 
和我之前用栈实现的算法相比，时间复杂度都是O(n)，
但由于不用栈储存数据，空间开销大大降低了。  
更重要的是操作上也大大简化，如果我当时想到这个算法的话手写应该没问题……

不过这个算法的灵感来自《编程珠玑》2.3节。
> 书到用时方恨少
